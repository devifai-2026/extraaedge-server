...
router.get('/config', requireRole(...MANAGER_TIER_ROLES), async (req, res, next) => {
  try {
...
router.post('/test', requireRole(...MANAGER_TIER_ROLES), validate({ body: z.object({ lead_id: z.string().uuid() }) }), async (req, res, next) => {
  try {
