...
export const assignSlot = async (req, res, next) => { try { ok(res, req, await service.assignSlot(req.tenant, req.user, req.params.id, req.body.student_id, req.body.slot_at, req.body.starts_at, req.body.ends_at), 201); } catch (e) { next(e); } };
export const assignSlots = async (req, res, next) => { try { ok(res, req, await service.assignSlots(req.tenant, req.user, req.params.id, req.body.assignments), 201); } catch (e) { next(e); } };
...
export const assignableHr = async (req, res, next) => { try { ok(res, req, await service.assignableHr(req.tenant, req.user, req.query.interview_id || null)); } catch (e) { next(e); } };
