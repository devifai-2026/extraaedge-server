...
router.post('/templates', requireRole(...MANAGER_TIER_ROLES), validate({ body: templateCreateSchema }), async (req, res, next) => {
  try {
...
router.put('/templates/:id', requireRole(...MANAGER_TIER_ROLES), validate({ params: idParam, body: templateUpdateSchema }), async (req, res, next) => {
  try {
...
router.post('/templates/:id/duplicate', requireRole(...MANAGER_TIER_ROLES), validate({ params: idParam }), async (req, res, next) => {
  try {
    const { rows } = await tenantQuery(
      req.tenant,
      `INSERT INTO email_templates (name, subject, body_html, body_text, variables, language, category, status, builder_type, is_visible, created_by)
